        ...
        <div className="grid grid-cols-3 gap-2">
            {data.map((result: any, i: number) => (
                <div key={result.timestamp + result.episode + i} className="p-1">
                    <Image src={getImgUrl(result)} alt={`${result.episode}: ${result.timestamp}`} width={400} height={240} className="box-border hover:outline hover:outline-8 hover:outline-theme-black" />
               </div>
            ))}
        </div>
    );
